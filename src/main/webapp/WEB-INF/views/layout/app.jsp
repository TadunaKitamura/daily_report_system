...
<body>
	<div id="wrapper">
		<div id="header">
			<div id="header_menu">
				...
				<c:if test="${sessionScope.login_employee != null}">
                    ...
                    <a href="<c:url value='?action=${actFav}&command=${commIdx}' />">マイいいねリスト</a>&nbsp;
                    <c:if test="${sessionScope.login_employee.adminFlag == AttributeConst.ROLE_ADMIN.getIntegerValue()}">
						<a href="<c:url value='?action=${actCon}&command=${commIdx}' />">確認リスト</a>&nbsp;
                    </c:if>
					<a href="<c:url value='?action=${actRep}&command=${commIdx}' />">いいねランキング</a>&nbsp;
                </c:if>
                </div>
				<c:if test="${sessionScope.login_employee != null}">
		...
		<div id="footer">by Dream Ship.</div>
	</div>
</body>
</html>